  {{toc numbered="false" scope="LOCAL" start="2"/}}
  ...
  **Submission forms**: the configuration file that defines your submission forms, input-forms.xml needs to be be extended with a number of new entry options.
  ...
  | dc.rights.uri | ali:license_ref  |  [http://creativecommons.org/licenses/by/3.0/igo/](http://creativecommons.org/licenses/by/3.0/igo/)|`` <ali:license_ref start_date="2015-01-20"> |
  |   |   |   | ``` http://creativecommons.org/licenses/by/3.0/igo/ ``` |
  |   |   |   | ``` </ali:license_ref> ``` |
  | dc.date.issued| ali:license_ref:startdate| 2015-01-20| ` <ali:license_ref start_date="2015-01-20"> 
  http://creativecommons.org/licenses/by/3.0/igo/
  </ali:license_ref>{{/code}}
  | dc.coverage| dc:coverage| Columbus, Ohio, USA; Lat: 39 57 N Long: 082 59 W|{{code}}<dc:coverage>
  Columbus, Ohio, USA; Lat: 39 57 N Long: 082 59 W
  </dc:coverage>{{/code}}
  | dc.description.abstract| dc:description| example item|{{code}}<dc:description>
  example item
  </dc:description>{{/code}}
  ...
  | dc.language.iso| dc:language | en-GB|{{code}}<dc:language>
  en-GB
  </dc:language>{{/code}}
  | dc.publisher| dc:publisher | PLOS ONE|{{code}}<dc:publisher>
  PLOS ONE
  </dc:publisher>{{/code}}
  | dc.relation.uri| dc:relation |http://datadryad.org/resource/doi:10.5061/dryad.tg469 |{{code}}<dc:relation>
  http://datadryad.org/resource/doi:10.5061/dryad.tg469
  </dc:relation>{{/code}}
  | dc.identifier.isbn| dc:source | 0-14-020652-3|{{code}}<dc:source>
  0-14-020652-3
  </dc:source>{{/code}} 
  | dc.identifier.issn| dc:source| 1456-2979 |{{code}}<dc:source>
  1456-2979
  </dc:source>{{/code}} 
  | dc.subject| dc:subject | example |{{code}}<dc:subject>
  example
  </dc:subject>{{/code}}
  | dc.title| dc:title| Title:Subtitle|{{code}}<dc:title>
  Title:Subtitle
  </dc:title>{{/code}}
  | dcterms.dateAccepted| dcterms:dateAccepted| 2015-02-10|{{code}}<dcterms:dateAccepted>
  2015-02-10
  </dcterms:dateAccepted>{{/code}}
  | rioxxterms.apc| rioxxterms:apc| paid|{{code}}<rioxxterms:apc>
  paid
  </rioxxterms:apc>{{/code}}
  | dc.contributor.author (first)| rioxxterms:author|Lawson, Gerry|{{code}}<rioxxterms:author id="http://orcid.org/0000-0002-1395-3092" first-named-author="true">
  Lawson, Gerry
  </rioxxterms:author>{{/code}}
  | dc.contributor.author| rioxxterms:contributor|Lawson, Gerry|{{code}}<rioxxterms:contributor id="http://orcid.org/0000-0002-1395-3092">
  Lawson, Gerry
  </rioxxterms:contributor>{{/code}}
  | rioxxterms.identifier.project| rioxxterms:project| 0123456789|{{code}}<rioxxterms:project rioxxterms:funder_name="Engineering and Physical Sciences Research Council" 
  rioxxterms:funder_id="http://dx.doi.org/10.13039/501100000266">
  EP/K023195/1
  </rioxxterms:project>{{/code}}
  | rioxxterms.funder| rioxxterms:project| Engineering and Physical Sciences Research Council|{{code}}<rioxxterms:project rioxxterms:funder_name="Engineering and Physical Sciences Research Council" 
  rioxxterms:funder_id="http://dx.doi.org/10.13039/501100000266">
  EP/K023195/1
  </rioxxterms:project>{{/code}}
  | dc.date.issued| rioxxterms:publication_date| 2015-02-15 |{{code}}<rioxxterms:publication_date>
  2015-02-15
  </rioxxterms:publication_date>{{/code}}
  | rioxxterms.type with dc.type 
   fallback| rioxxterms:type| Book|{{code}}<rioxxterms:type>
  Book
  </rioxxterms:type>{{/code}}
  | rioxxterms.version| rioxxterms:version| AO|{{code}}<rioxxterms:version>
  AO
  </rioxxterms:version>{{/code}}
  | rioxxterms.versionofrecord| rioxxterms:version_of_record|http://dx.doi.org/10.1006/jmbi.1995.0238|{{code}}<rioxxterms:version_of_record>
  http://dx.doi.org/10.1006/jmbi.1995.0238
  </rioxxterms:version_of_record>{{/code}}
  ...
  | format| dc:format| application/pdf|{{code}}<dc:format>
  application/pdf
  </dc:format>{{/code}}
  | url| dc:identifier|https://example.com/dspace/bitstream/123456789/10/1/example.pdf|{{code}}<dc:identifier>
  https://example.com/dspace/bitstream/123456789/10/1/example.pdf
  </dc:identifier>{{/code}}
  | embargo| ali:free_to_read| 2015-08-27|{{code}}<ali:free_to_read start_date="2015-08-27">
  </ali:free_to_read>{{/code}}
  ...
  **__Important note__**: Below, we will explain you how to apply the patch to your existing installation. This will affect your source code. Before applying a patch, it is **always** recommended to create backup of your DSpace source code.
  ...
  In order to apply the patch, you will need to locate the **DSpace source code** on your server. That source code directory contains a directory “ *dspace* ”, as well as the following files: " _LICENSE_", " _NOTICE_ ", " _README_ ", ....
  ...
  * Git should be installed on the machine. The patch will be applied using several git commands as indicated in the next section. 
  ...
  ## Submission forms based on @mire template ##
  ...
  **Rioxxterms:project**
  ...
  ## Errors during the Patch Installation process ##
